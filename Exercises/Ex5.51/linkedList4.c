...
int linkedList_insert_at_begin(linkedList* l, node_data val) {
  ...
  if( linkedList_isempty(l) ) {
    l->head = var; l->head->next = NULL;
  } else {
    var->next = l->head; l->head=var;
  }
}

/*----------------------------------------------------------------------
 * Returns in p the pointer to the pointer of element at index n We do
 * this so that we can actualy modify the pointer of the element, for
 * example if we want to modify l->head, we need to have a pointer to
 * l->head
 *----------------------------------------------------------------------*/

int linkedList_at_index(linkedList *l, size_t n, linkedList_node ***p) {
  linkedList_node **var;
  size_t count = 0;
  for(count = 0, var = &(l->head);
      ((*var != NULL) && (count < n));
      var = &((*var)->next), count++);
  if(count == n) {
    *p = var; return 1;
  } else {
    line; printf("\t\tThe LinkedList_Node Number Is Not Found...\n"); line;
    *p = NULL; return 0;
  }
}

int linkedList_insert_at_index(linkedList *l, node_data val, int pos) {
  linkedList_node *var, **tmp;
  if( linkedList_at_index(l,pos,&tmp) ) {
    var = (linkedList_node*) malloc( sizeof(linkedList_node) );
    var->data = val; var->next = *tmp; *tmp = var;
  }
}

int linkedList_search(linkedList *l, node_data nd, linkedList_node ***p, size_t *count) {
  linkedList_node **var;
  
  for(var = &(l->head), *count = 0;
      ((*var != NULL) && (!l->equal((*var)->data,nd)));
      var = &((*var)->next), *count = *count + 1);
  if(*var != NULL) {
    *p = var; return 1;
  } else {
    line; printf("\t\tLinkedList: Match not found...\n"); line;
    *p = NULL; return 0;
  }
}

int linkedList_insert_at_match(linkedList *l, node_data val, node_data loc) {
  linkedList_node *var, **tmp; size_t count;
  if ( linkedList_search(l, loc, &tmp, &count) ) {
    var = (linkedList_node*) malloc( sizeof(linkedList_node) );
    var->data = val; var->next = *tmp; *tmp = var;
  }
}

int linkedList_insert_after_match(linkedList *l, node_data val, node_data loc) {
  int flag=0;
...
int main() {
  ...
  do {
    ...
    case 1:
      ...
      scanf("%d",&val);
      linkedList_insert_at_begin(l.self,l.new(1,val));
    ...
    case 2:
      ...
      scanf("%d",&val);
      locI = l.new(1,loc);
      linkedList_insert_after_match(l.self,l.new(1,val),locI);
      l.destroy(locI);
      linkedList_display(l.self);
      break;
    case 3:
      printf("Enter a Position At You Want To Insert Val: ");
      scanf("%d",&pos);
      printf("Enter Value to Insert a linkedList_node: ");
      scanf("%d",&val);
      linkedList_insert_at_index(l.self, l.new(1,val), pos);
      linkedList_display(l.self);
      break;
    case 4:
      printf("Enter Value to Insert a linkedList_node: ");
